...
def eval(model, val_iter, attr='pred_F', is_save=False, phase='test'):
    model.eval()
    total_pred = []
    total_label = []
    total_miss_type = []
    for i, data in enumerate(val_iter):  # inner loop within one epoch
        ...
        model.test()
        pred = getattr(model, attr).argmax(dim=1).detach().cpu().numpy()
        label = data['label']
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        # eval pred_T
        acc, uar, f1, cm = eval(model, val_dataset, 'pred_T')
        logger.info('Val result on pred_T of epoch %d / %d acc %.4f uar %.4f f1 %.4f' % (epoch, opt.niter + opt.niter_decay, acc, uar, f1))
        logger.info('\n{}'.format(cm))
        # eval pred_F
        acc, uar, f1, cm = eval(model, val_dataset, 'pred_F')
        logger.info('Val result on pred F of epoch %d acc %.4f uar %.4f f1 %.4f' % (epoch, acc, uar, f1))
        logger.info('\n{}'.format(cm))
    ...
    logger.info('Best eval epoch %d found on F with uar %f' % (best_eval_epoch, best_eval_uar))
    # test
    logger.info('Loading best model found on val set: epoch-%d' % best_eval_epoch)
    model.load_networks(best_eval_epoch)
    _ = eval(model, val_dataset, attr='pred_F', is_save=True, phase='val')
    acc, uar, f1, cm = eval(model, tst_dataset, attr='pred_F', is_save=True, phase='test')
