...
class NewCRASingleTeacherModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--A_teacher_path', type=str, default='None')
        parser.add_argument('--V_teacher_path', type=str, default='None')
        parser.add_argument('--L_teacher_path', type=str, default='None')
    ...
    def init_from_exist_model(self, opt):
        print('Init parameter net A from {}'.format(opt.A_teacher_path))
        print('Init parameter net V from {}'.format(opt.V_teacher_path))
        print('Init parameter net L from {}'.format(opt.L_teacher_path))
        A_teacher_path = os.path.join(opt.A_teacher_path, str(opt.cvNo))
        V_teacher_path = os.path.join(opt.V_teacher_path, str(opt.cvNo))
        L_teacher_path = os.path.join(opt.L_teacher_path, str(opt.cvNo))
        A_teacher_config_path = os.path.join(opt.A_teacher_path, 'train_opt.conf')
        V_teacher_config_path = os.path.join(opt.V_teacher_path, 'train_opt.conf')
        L_teacher_config_path = os.path.join(opt.L_teacher_path, 'train_opt.conf')
        A_teacher_config = self.load_from_opt_record(A_teacher_config_path)
        V_teacher_config = self.load_from_opt_record(V_teacher_config_path)
        L_teacher_config = self.load_from_opt_record(L_teacher_config_path)
        A_teacher_config.isTrain = False                             # teacher model should be in test mode
        V_teacher_config.isTrain = False                             # teacher model should be in test mode
        L_teacher_config.isTrain = False                             # teacher model should be in test mode
        A_teacher_config.gpu_ids = opt.gpu_ids                       # set gpu to the same
        V_teacher_config.gpu_ids = opt.gpu_ids                       # set gpu to the same
        L_teacher_config.gpu_ids = opt.gpu_ids                       # set gpu to the same
        self.A_teacher_model = EarlyFusionMultiModel(A_teacher_config)
        self.A_teacher_model.load_networks_cv(A_teacher_path)
        self.A_teacher_model.cuda()
        self.A_teacher_model.eval()
        self.V_teacher_model = EarlyFusionMultiModel(V_teacher_config)
        self.V_teacher_model.load_networks_cv(V_teacher_path)
        self.V_teacher_model.cuda()
        self.V_teacher_model.eval()
        self.L_teacher_model = EarlyFusionMultiModel(L_teacher_config)
        self.L_teacher_model.load_networks_cv(L_teacher_path)
        self.L_teacher_model.cuda()
        self.L_teacher_model.eval()
    ...
    def post_process(self):
        ...
        self.netA.load_state_dict(f(self.A_teacher_model.netA.state_dict()))
        self.netV.load_state_dict(f(self.V_teacher_model.netV.state_dict()))
        self.netL.load_state_dict(f(self.L_teacher_model.netL.state_dict()))
    ...
    def forward(self):
        ...
        if self.isTrain:
            with torch.no_grad():
                self.T_embd_A = self.A_teacher_model.netA(self.acoustic_reverse)
                self.T_embd_L = self.L_teacher_model.netL(self.lexical_reverse)
                self.T_embd_V = self.V_teacher_model.netV(self.visual_reverse)
