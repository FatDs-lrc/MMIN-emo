...
class AlignedEFTestModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        if 'A' in self.modality:
            ...
            self.netA = LSTMEncoder(opt.input_dim_a, opt.embd_size_a, opt.embd_method, pool_len=22)
            
        # lexical model
        if 'L' in self.modality:
            self.model_names.append('L')
            self.netL = LSTMEncoder(opt.input_dim_l, opt.embd_size_l, opt.embd_method, pool_len=22)
            
        # visual model
        if 'V' in self.modality:
            self.model_names.append('V')
            self.netV = LSTMEncoder(opt.input_dim_v, opt.embd_size_v, opt.embd_method, pool_len=22)
