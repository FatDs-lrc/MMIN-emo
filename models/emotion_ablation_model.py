...
class EmotionAblationModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.add_argument('--fusion_layers', type=str, default='128,128', help='256,128 for 2 layers with 256, 128 nodes respectively')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['CE']
        self.model_names = ['C']
        fusion_layers = list(map(lambda x: int(x), opt.fusion_layers.split(',')))
        self.netC = FcClassifier(128, fusion_layers, opt.output_dim, dropout=opt.dropout_rate, use_bn=opt.bn)
