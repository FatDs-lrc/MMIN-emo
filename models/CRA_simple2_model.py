...
class CRASimple2Model(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        # miss feat extractor
        self.netC_miss = FcClassifier(384, fusion_layers, opt.output_dim, dropout=opt.dropout_rate, use_bn=opt.bn)
        # AE model
    ...
    def forward(self):
        ...
        # calc embd_pred
        self.logits_miss, self.emb_pred = self.netC_miss(self.feat_fusion_miss)
        # get fusion outputs for missing modality
        self.logits, _ = self.netC(torch.cat([self.emb_pred, self.latent], dim=-1))
    ...
    def backward(self):
        ...
        self.loss_CE = self.ce_weight * (self.criterion_ce(self.logits, self.label) + \
                                         self.criterion_ce(self.logits_miss, self.label))
