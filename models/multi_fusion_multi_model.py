...
class MultiFusionMultiModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        # self.loss_names = ['CE']
        self.loss_names = ['CE_A', 'CE_L', 'CE_V', 'CE_F']
        ...
        if 'V' in self.modality:
            ...
            self.netV_C = FcClassifier(opt.embd_size_v, fusion_layers, opt.output_dim, dropout=opt.dropout_rate)
            # self.netV = TextCNN(opt.input_dim_v, opt.embd_size_v)
            
        if self.isTrain:
            self.criterion_ce = torch.nn.NLLLoss()
            self.criterion_ce2 = torch.nn.CrossEntropyLoss()
    ...
    def backward(self):
        ...
        self.loss_CE_F = self.criterion_ce2(self.ef_logits, self.label)
        self.loss_CE_F.backward(retain_graph=True)

        self.loss_CE_A = self.criterion_ce2(self.logits_A, self.label)
        self.loss_CE_A.backward()

        self.loss_CE_L = self.criterion_ce2(self.logits_L, self.label)
        self.loss_CE_L.backward()

        self.loss_CE_V = self.criterion_ce2(self.logits_V, self.label)
        self.loss_CE_V.backward()
