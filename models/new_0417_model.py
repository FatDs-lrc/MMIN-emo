...
class New0417Model(BaseModel):
    ...
    def __init__(self, opt):
        """new translation model + AVL 映射
        Parameters:
            opt (Option class)-- stores all the experiment flags; needs to be a subclass of BaseOptions
        """
        super().__init__(opt)
        self.loss_names = ['CE_T', 'CE_F', 'mse', 'cycle']
        self.model_names = ['A', 'V', 'L', 'map', 'C_T', 'C_F', 'AE', 'AE_cycle']
        ...
        self.netC_F = FcClassifier(fusion_layers[-1] + AE_layers[-1], [128, 64], opt.output_dim, dropout=opt.dropout_rate, use_bn=opt.bn)

        # define net map
        self.netmap = FcEncoder(opt.fusion_size, [256, 256, 384], 0, use_bn=False)
        # acoustic model
    ...
    def forward(self):
        ...
        self.feat_fusion_miss_map = self.netmap(self.feat_fusion_miss)
        self.recon_fusion, self.latent = self.netAE(self.feat_fusion_miss_map)
    ...
    def backward(self):
        ...
        self.loss_cycle = self.cycle_weight * self.criterion_mse(self.feat_fusion_miss_map.detach(), self.recon_cycle)
