...
class ModalityMissAEModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        """
        super().__init__(opt)
        self.loss_names = ['CE', 'mmd']
        self.model_names = ['A', 'V', 'L', 'C', 'AE', 'Miss']
        
        # define network autoencoder
        AE_layers = list(map(lambda x: int(x), opt.AE_layers.split(',')))
        # self.netAE = FcEncoder(opt.fusion_size, [opt.fusion_size, opt.fusion_size], dropout=opt.dropout_rate, use_bn=opt.bn)
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            self.full_logits, _ = self.netC(self.feat_fusion)
            # mix missing modality logits and full modality logits
            _lambda = min(1 - self.cur_epoch / self.niter_annealing, 0)
            self.logits = _lambda * self.full_logits + (1-_lambda) * self.logits
    ...
    def backward(self):
        ...
        if self.mmd_loss_type == 'CST':
            sample_num = 4
            self.loss_mmd = self.mmd_weight * self.criterion_mmd(self.recon_fusion, self.feat_fusion, torch.ones(batch_size).to(self.device))
            for random_idx in random.sample(range(1, batch_size-1), sample_num):
                re_rank_feat = torch.cat([self.feat_fusion[random_idx:], self.feat_fusion[:random_idx]])
        ...
        #     self.loss_mmd = self.mmd_weight * self.criterion_mmd(self.recon_fusion, self.feat_fusion) 
        ...
        loss = self.loss_CE + self.loss_mmd
        loss.backward()
