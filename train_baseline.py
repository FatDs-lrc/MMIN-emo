...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        # # eval teacher
        # acc, uar, f1, cm = eval(model.teacher_model, val_dataset)
        # logger.info('Teacher result of epoch %d / %d acc %.4f uar %.4f f1 %.4f' % (epoch, opt.niter + opt.niter_decay, acc, uar, f1))
        # logger.info('\n{}'.format(cm))
    ...
    logger.info('Best eval epoch %d found with f1 %f' % (best_eval_epoch, best_eval_f1))
    # test
